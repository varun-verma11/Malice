...
public class StatementChecker
{
	...
	private static void validateBecame(Tree node, SymbolTable symbolTable)
	{
		...
		{
			if (SemanticsUtils.getValueType(node.getChild(1), symbolTable) != DATA_TYPES.NUMBER)
			{
				System.err.println("Line " + node.getLine() + ": "
						+ node.getCharPositionInLine()
					...
					!= DATA_TYPES.valueOf("ARRAY_" + SemanticsUtils.
							getValueType(node.getChild(3), symbolTable))) 
			{
				System.err.println("Line " + node.getLine() + ": "
						+ node.getCharPositionInLine()
						+ " : Type of array and initialising value are " +
								"different" );
			}
		} else if ((symbolTable.lookup(node.getChild(0).getText())).getType() 
				!= SemanticsUtils
